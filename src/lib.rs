...
impl Statistics {
    ...
    pub fn from<'a>(contents: &'a str) -> Result<Statistics, ParseIntError> {
        ...
        for (line_index, line) in contents.lines().enumerate() {
            ...
            for (chr_index, chr) in line.chars().enumerate() {
                if chr == ',' {
                    ...
                    stats.parse_word(val, line_index, chr_index)?;
                }
            }
            ...
            if left_ptr != line.len() {
                ...
                if !val.is_empty() {
                    stats.parse_word(val, line_index, left_ptr)?;
                }
            }
        }
    ...
    fn parse_word(
        ...
        self.min = cmp::min(self.min, num);

        let count = self.num_freq_map.entry(num).or_insert(0);
        *count += 1;
...
impl HistogramInt {
    pub fn new(args: &Args) -> Result<HistogramInt, Box<dyn Error>> {
        let contents = fs::read_to_string(args.filename.clone())?;
        let statistics = Statistics::from(&contents)?;
...
mod tests {
    ...
    fn test_uniform_10000_range_1_100_bin_10() {
        ...
        let histo = HistogramInt::new(&args).unwrap();
    ...
    fn test_uniform_10000_starting_larger_than_ending() {
        ...
        let histo = HistogramInt::new(&args);
        assert!(histo.is_err())
    }
}
